...
function parse(x) {
    switch (type(x)) {
        ...
        case 'Date':
            return x
        case 'String':
            return dateFromString(x)
        case 'Number':
            return new Date(resolveTimestamp(x))
        case 'Null':
        case 'Undefined':
            return new Date()
    }
...
function dateFromString(s) {
    const r = /^(april|august|december|february|january|june|july|march|may|november|october|september|apr|aug|dec|feb|jan|jun|jul|mar|may|nov|oct|sep) *(\d+)?/i
    ...
    if (m = match(s, r)) {
        ...
        const r = RegExp('^' + monthName, 'i')
        const finder = (month) => {
            return r.test(month)
        }
        const monthIndex = variables.months.findIndex(finder)
        return new Date(YEAR, monthIndex, day)
    }

    const dateRE = /^(\d+)[^\d\s](\d+)(?:[^\d\s](\d+))?/
    if (m = match(s, dateRE)) {
        let [a, b, c] = m 
        if (c == null) {
            c = YEAR
        ...
        return new Date(c, Number(b) + 1, Number(a))
    }
    ...
    if (m = match(s, /^(\d+):(\d+)([ap]m)?/i)) {
        let [hour, min, ampm] = m.map(toNumber)
        if (ampm == 'pm' || ampm == '') {
            hour += 12
        }
        return new Date(YEAR, MONTH - 1, DAY, hour, min)
    }
    else if (m = match(s, /^(\d+)([ap]m)/i)) {
        let [hour, ampm] = m.map(toNumber)
        if (ampm == 'pm' || ampm == '') {
            hour += 12
        }
