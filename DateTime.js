...
class DateTime {
    ...
    get(key) {
        switch(key) {
            case 'day': return getDay(this.date)
...
function getMonth(date) {
    ...
    return {
        number: index + 1,
        name: variables.monthNames[index],
        index,
    }
}
function getDay(date) {
    const index = date.getDay();
    const name = variables.dayNames[index];
